...
public class UserStoreTest {
    ...
    @Before
    public void init() {
        values = new UserStore();
        values.add(new User(ID_1));
        values.add(new User(ID_2));
        values.add(new User(ID_3));
        values.add(new User(ID_4));
    }
    ...
    public void whenReplaceSequential() {
        assertThat(values.replace(ID_4, new User(ID_REPLACE)), is(true));
        assertThat(values.replace(ID_4, new User(ID_REPLACE)), is(false));
        assertThat(values.findById(ID_4), is((Base) null));
    }
}